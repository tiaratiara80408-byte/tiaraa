...
const DashboardView: React.FC = () => {
  ...
  const ActivityItem: React.FC<{item: any}> = ({ item }) => {
    if (item.type === 'match_result') {
      ...
      return (
        <div className="flex items-start space-x-3 p-3 rounded-lg hover:bg-slate-700/50">
          {isWin ? <CheckCircleIcon className="h-5 w-5 text-green-400 mt-1" /> : <ExclamationTriangleIcon className="h-5 w-5 text-red-400 mt-1" />}
          <div>
            <p className="font-semibold text-sm">Hasil Pertandingan</p>
    ...
    if (item.type === 'training') {
      return (
        <div className="flex items-start space-x-3 p-3 rounded-lg hover:bg-slate-700/50">
          ...
          <div>
            <p className="font-semibold text-sm">Latihan: {item.data.title}</p>
            <p className="text-xs text-gray-400">{new Date(item.data.date).toLocaleDateString('id-ID', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
          </div>
        </div>
      );
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400">Selamat datang kembali, Pelatih!</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
       ...
       <div>
        <h3 className="text-xl font-bold mb-4">Aktivitas Tim</h3>
      ...
      <div>
        <h3 className="text-xl font-bold mb-4">Pertandingan Mendatang</h3>
        <div className="space-y-4">
          {upcomingMatches.length > 0 ? (
            upcomingMatches.slice(0, 1).map(match => <MatchCard key={match.id} match={match} />)
          ) : (
            <div className="bg-slate-800 rounded-lg p-6 text-center">
              <p className="text-gray-400">Tidak ada pertandingan mendatang.</p>
              <p className="text-sm text-gray-500 mt-1">Saatnya jadwalkan latih tanding!</p>
            </div>
          )}
        </div>
      </div>
