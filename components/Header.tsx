  ...
  return (
    <header className="flex items-center justify-between p-4 bg-slate-800/80 backdrop-blur-sm sticky top-0 z-10">
      <div className="flex items-center space-x-2">
        ...
        <h1 className="text-xl font-bold">VolleyApp</h1>
      </div>
      <div className="flex items-center space-x-2">
        <button onClick={onCartClick} className="relative p-2 rounded-full hover:bg-slate-700" aria-label="Buka Keranjang">
            <ShoppingCartIcon className="h-6 w-6" />
            {cartItemCount > 0 && (
            <span className="absolute -top-1 -right-1 h-5 w-5 bg-red-600 text-white text-xs font-bold rounded-full flex items-center justify-center border-2 border-slate-800">
                {cartItemCount}
            </span>
            )}
        </button>
